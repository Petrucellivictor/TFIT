...
- CORS: `src/proxy.ts` sets `Access-Control-Allow-Origin` (reflecting the request's `Origin` header), `-Methods`, and `-Headers`, and answers `OPTIONS` preflight requests directly, for every route. This was added once a browser-based client (the Expo web preview) started hitting the API and had every write silently blocked — the preflight itself returned 204, but without these headers the browser never sent the real request at all. Permissive CORS doesn't weaken auth here: the API is bearer-token authenticated (`Authorization` header), not cookie-based, so a third-party page still can't forge a valid Clerk session token for a user just because its origin is allowed to make the request — CORS is a browser-enforced boundary for *credentialed, cookie-based* requests, which this API doesn't use.
