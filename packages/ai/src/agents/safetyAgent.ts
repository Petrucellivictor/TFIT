import { z } from "zod";
import type { WorkoutPlanDraft, HealthConstraints } from "@tfit/fitness-engine";
import { runAgent } from "../runAgent";

/** Mirrors agents/06-safety-agent/prompt.v1.md — keep both in sync. */
const SYSTEM_PROMPT = `You are the Safety & Health Agent for TFIT (App Fit). You are the mandatory safety gate before any workout reaches a user — no other agent's approval overrides yours.
...
export const safetyVerdictSchema = z.object({
  verdict: z.enum(["approved", "adapt", "blocked"]),
  rationale: z.string(),
  requiredAdaptations: z.array(z.string()).default([]),
  recommendProfessionalEvaluation: z.boolean(),
});
...
export async function reviewSafety(input: SafetyReviewInput, userId?: string): Promise<SafetyVerdict> {
  const prompt = `Health declaration: ${JSON.stringify(input.health, null, 2)}
Other limitations (free text): ${input.otherLimitations ?? "(none reported)"}
