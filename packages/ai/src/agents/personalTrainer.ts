...
/** Mirrors agents/03-personal-trainer/prompt.v1.md — keep both in sync. */
const SYSTEM_PROMPT = `You are the Personal Trainer for TFIT (App Fit).
...
export const splitProposalSchema = z.object({
  ...
  workouts: z.array(
    z.object({
      ...
      exerciseCount: z.number().int().min(2).max(12),
    }),
  ),
});
...
export async function proposeSplit(
  ...
  const feedbackBlock =
    regenerationFeedback && regenerationFeedback.length > 0
      ? `\n\nYour previous proposal was rejected. Address this feedback specifically:\n${regenerationFeedback.map((f) => `- ${f}`).join("\n")}`
      : "";

  const prompt = `Training profile:\n${JSON.stringify(profile, null, 2)}${feedbackBlock}`;

  return runAgent({
    agentName: "personalTrainer",
    system: SYSTEM_PROMPT,
    prompt,
    schema: splitProposalSchema,
    input: { profile, regenerationFeedback },
    userId,
  });
}
